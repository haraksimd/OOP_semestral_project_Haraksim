...
public class homepageController extends loginController{
    ...
    public int getIndexAnimal(){
        int selectedItem;
        selectedItem = animalListTable.getSelectionModel().getSelectedItem().getID();
    ...
    public void fillToDoTable(){
        ...
        try {
            ...
            preparedCountStatement = connectDB.prepareStatement(countQuery);
            preparedCountStatement.setInt(1, indexSS);
            ResultSet resultSetSize = preparedCountStatement.executeQuery();
            if(resultSetSize.next())
                size = resultSetSize.getInt(1);

            preparedQuery = connectDB.prepareStatement(sqlQuery);
            preparedQuery.setInt(1, indexSS);
            ResultSet data = preparedQuery.executeQuery();
            data.next();
    ...
    public void fillChoiceBox(){
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                choiceBox.getItems().add(list[i].getMeno());
            }
    ...
    public void addEmployee() throws Exception{
        ...
        Stage window = (Stage) addEmployeeButton.getScene().getWindow();
        window.setScene(new Scene(root));
    }

    public void addAnimal() throws Exception{
        Parent root = FXMLLoader.load(getClass().getResource("../views/addAnimal.fxml"));
    ...
    public void addMessage() throws Exception{
        ...
        Stage window = (Stage) messageBtn.getScene().getWindow();
        window.setScene(new Scene(root));
    }
